...
| CI covers format, tests, coverage, security, OpenAPI, Compose, Docker build, and pinned validation tool versions. | `.github/workflows/ci.yml`, `go.mod`, `Makefile` | Done | Docker build validation is in CI; Go toolchain and Redocly CLI versions are explicit to reduce validation drift. |
