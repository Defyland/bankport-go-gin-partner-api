...
| Observability has metrics, logs, traces, bounded identity headers, dashboard, alerts, and runbooks. | `internal/observability/metrics.go`, `docs/architecture/observability.md`, `deployments/grafana/dashboards/bankport-partner-api.json`, `deployments/prometheus/alerts.yml`, `docs/runbooks/`, `TestMetricsUseRoutePatternForAccountIDs`, `TestRequestIdentityRejectsUnsafeCallerSuppliedIDs` | Done | Metrics and traces use route patterns to avoid account-id cardinality; caller-supplied request IDs are bounded and normalized; trace exporter and Alertmanager routing are planned. |
